...
os.environ['THEANO_FLAGS'] = 'device=cuda{}'.format(params.gpunum)
...
TileFolder = params.inputpath
FileName = params.inputfile
lmdb_reader = LmdbReader(path=TileFolder, name=FileName)

# original pred result outfile
# pred_result_out = '/media/mycloud/outfile_TCGA-2F-A9KQ-01Z-00_ref_lmdb_input'
pred_result_out = params.out

"""Read lmdb meta info"""
metas = lmdb_reader.get_meta_info()

# patch_on_slide_size = 219
patch_on_slide_size = int(metas["patch_on_slide_size"])
patch_size = 100
start_x = 1
start_y = 1
# pixel_X = 110592
pixel_X = int(metas["slide_X_pixels"])
# pixel_Y = 77312
pixel_Y = int(metas["slide_Y_pixels"])

CNNModel = './cnn_lym_model.pkl'


lmdb_todo_list = []
while start_y < pixel_Y:
    while start_x < pixel_X:
        filename = str(start_x)+'_'+str(start_y)+'_'+str(patch_on_slide_size)+'_'+str(patch_size)
        lmdb_todo_list.append(filename)
        start_x += patch_on_slide_size
    start_x = 1
    start_y += patch_on_slide_size
...
def load_data(todo_list, rind):
    ...
    for fn in todo_list:
        ...
        png_pw = float(fn.split('_')[3])
        # png_pw = float(fn.split('_')[3].split('.png')[0])

        # png = np.array(Image.open(full_fn).convert('RGB'))
        # load from LMDB
        png = lmdb_reader.lmdb_txn.get(fn.encode('ascii'))
        png = np.array(pickle.loads(png))

        # their original meaning is sampling from their 1000*1000 image
...
def val_fn_epoch_on_disk(classn, val_fn):
    ...
    # load todo_list from lmdb file:
    todo_list = lmdb_todo_list
